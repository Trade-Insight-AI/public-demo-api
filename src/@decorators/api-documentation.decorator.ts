...
export function ApiDocumentation(options: IApiDocumentationOptions) {
  return (
    ...
    // When applied to a method, apply the tags to the controller class to override default naming
    if (propertyKey && options.tags && options.tags.length > 0) {
      const controllerClass = target.constructor;
      options.tags.forEach((tag) => {
        ApiTags(tag)(controllerClass);
      });
    }
